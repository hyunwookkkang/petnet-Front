...
const PlaceSearch = () => {
  ...
  const [places, setPlaces] = useState([]); // 검색 결과 (빈 배열로 초기화)
  const [map, setMap] = useState(null); // Google Maps 객체
    ...
    const handleSearch = async () => {
        ...
        const apiUrl =
        searchType === "지역"
            ? `/api/map/places/search/ctyprvnSignguNm?ctyprvnSignguNm=${searchKeyword.trim()}`
            : `/api/map/places/search/fcltyNm?fcltyNm=${searchKeyword.trim()}`;
        ...
        if (Array.isArray(data)) {
            setPlaces(data); // 데이터가 배열일 경우만 상태 업데이트
