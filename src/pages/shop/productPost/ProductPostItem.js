...
const ProductPostItem = ({ post, isUserPost, onEdit, onDelete, onClick }) => {
  ...
  return (
    <div className="post-item" onClick={() => onClick(post)}>
      ...
      <div className="post-header">
        <strong>
          작성자: {post.userId} {authorId === userId && "(본인)"}
        </strong>
        <span>작성일: {post.addDate || "날짜 없음"}</span>
        <span>수정일: {post.updateDate || "날짜 없음"}</span>
      </div>
